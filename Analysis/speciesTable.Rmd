...
## Ditch everything above and pull and output from the main analysis script
#====================================================================
library(pander)
# Generate total counts for each species and common name
cnt.tab<-aggregate(catch_cpue ~ TAXONOMIC + common_name, data=complete.dat, FUN=mean)

# Generate total biomass for each taxonomic name
bio.tab<-aggregate(cpue ~ TAXONOMIC , data=complete.dat, FUN=mean)

# Generate a number of years present
...
# Rename columns for pretty
pandoc.table(complete.tab, split.tables=Inf, round=4)
```
