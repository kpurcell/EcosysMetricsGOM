...
To evaluate temporal patterns in the mean length of the community we evaluated an overall time-series for all 65 taxonomic entities involved in our study, as well as time-series for our four bycatch categories (Catch, High, Rare, Never).  All calculations were conducted on post-1987 data due to previously stated inconsistencies in sampling protocols, and only measurements in which total length (TL) were preformed (`r (length(sfDat$meas_typ[sfDat$meas_typ=="TL"])/length(sfDat$meas_typ))*100`% of observations).  
