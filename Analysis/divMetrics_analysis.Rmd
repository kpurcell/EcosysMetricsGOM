...
sum.la.S<-as.data.frame(lowess(sum.la.agg$S~sum.la.agg$YR))
sum.la.d<-as.data.frame(lowess(sum.la.agg$d~sum.la.agg$YR))
sum.la.J<-lowess(sum.la.agg$J~sum.la.agg$YR))
sum.la.shan<-as.data.frame(lowess(sum.la.agg$shan~sum.la.agg$YR))
sum.la.simp<-as.data.frame(lowess(sum.la.agg$simp~sum.la.agg$YR))
sum.la.N1<-as.data.frame(lowess(sum.la.agg$N1~sum.la.agg$YR))
sum.la.N2<-as.data.frame(lowess(sum.la.agg$N2~sum.la.agg$YR))  
 
sum.tx.S<-as.data.frame(lowess(sum.tx.agg$S~sum.tx.agg$YR))
sum.tx.d<-as.data.frame(lowess(sum.tx.agg$d~sum.tx.agg$YR))
sum.tx.J<-as.data.frame(lowess(sum.tx.agg$J~sum.tx.agg$YR))
sum.tx.shan<-as.data.frame(lowess(sum.tx.agg$shan~sum.tx.agg$YR))
sum.tx.simp<-as.data.frame(lowess(sum.tx.agg$simp~sum.tx.agg$YR))
sum.tx.N1<-as.data.frame(lowess(sum.tx.agg$N1~sum.tx.agg$YR))
sum.tx.N2<-as.data.frame(lowess(sum.tx.agg$N2~sum.tx.agg$YR))

fall.la.S<-as.data.frame(lowess(fall.la.agg$S~fall.la.agg$YR))
fall.la.d<-as.data.frame(lowess(fall.la.agg$d~fall.la.agg$YR))
fall.la.J<-as.data.frame(lowess(fall.la.agg$J~fall.la.agg$YR))
fall.la.shan<-as.data.frame(lowess(fall.la.agg$shan~fall.la.agg$YR))
fall.la.simp<-as.data.frame(lowess(fall.la.agg$simp~fall.la.agg$YR))
fall.la.N1<-as.data.frame(lowess(fall.la.agg$N1~fall.la.agg$YR))
fall.la.N2<-as.data.frame(lowess(fall.la.agg$N2~fall.la.agg$YR))

fall.tx.S<-as.data.frame(lowess(fall.tx.agg$S~fall.tx.agg$YR))
fall.tx.d<-as.data.frame(lowess(fall.tx.agg$d~fall.tx.agg$YR))
fall.tx.J<-as.data.frame(lowess(fall.tx.agg$J~fall.tx.agg$YR))
fall.tx.shan<-as.data.frame(lowess(fall.tx.agg$shan~fall.tx.agg$YR))
fall.tx.simp<-as.data.frame(lowess(fall.tx.agg$simp~fall.tx.agg$YR))
fall.tx.N1<-as.data.frame(lowess(fall.tx.agg$N1~fall.tx.agg$YR))
fall.tx.N2<-as.data.frame(lowess(fall.tx.agg$N2~fall.tx.agg$YR))
...
This might need to be a seperate analysis.  Also just found some citations to species time curves ```r citep("10.1890/05-0067")```,```r citep("10.1046/j.1461-0248.2003.00497.x")```,```r citep("10.1890/05-0067")``` which could be an interesting way to determine the stability of the ecosystem and if we are seeing changes before and after large distrubtions in the ecosystem.  Perhaps I should revisit the Lorenz Curves (univariate:Gini index) from ```r citep("10.1139/f03-076")``` as a parameter but that uses catch, abundance, and distributional area so I am not sure that I can make the connections with the give resolution of data.  
